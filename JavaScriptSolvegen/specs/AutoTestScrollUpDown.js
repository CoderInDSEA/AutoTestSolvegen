describe('auto tests for testing text animation on scroll down and up',  () => {
    it('should expanding first image', async () => {
        const FirtParagraphText = await $('.vertical-pane-container-active #text_5abe7f3fb8987 span')
        const FirstLetterAnimationText = await $('#text_5abe7f3fb8af0 .n2-text-animated:nth-child(1) span:nth-child(1)')
        const LastParagraphText = await $('.vertical-pane-container-active #text_5abe7f3fb8c5b span')

        await browser.url('https://raminjshsdgadfaddasjhhbjkafaadfbjsd.stage.storied.co/components-test-app/text-animations-scroll/')
